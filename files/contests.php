    ...
    if (isset($_GET['code'])) {
        ...
        if ($contest) {
            ...
            <script type='text/javascript'>
                ...
                var etime = <?php echo $contest['endtime'] - time(); ?>;
                ...
                function timer() {
                    if (ctime > 0) {
                        ...
                        $("div#contesttimer").html("<h4>Ends in: " + parseInt(etime / 3600) + ":" + zeroPad(parseInt((etime / 60)) % 60,2) + ":" + zeroPad(etime % 60, 2) + "</h4>");
                        etime--;
                        window.setTimeout("timer();", 1000);
                    }
                    else{
                        $("div#contesttimer").html("<h4>Contest is Over.</h4>");
                    }
                }
                timer();
            </script>
            <?php
            echo "<div class='contestheader'><center><h1>$contest[name]</h1></center><div id='contesttimer'><h4>Contest is Over.</h4></div></div>";
            ...
            echo "<table class='table'><tr><th>Name</th><th>Score</th><th>Code</th><th>Submissions</th></tr>";
            if ($prob) {
				foreach ($prob as $row) {
